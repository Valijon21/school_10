...
const subjectLabels: Record<string, string> = { all: 'Barchasi', matematika: 'Matematika', 'ona-tili': 'Ona tili', 'ingliz-tili': 'Ingliz tili', informatika: 'Informatika', tarix: 'Tarix' }
...
useHead({
  title: 'O\'qituvchilar | 10-Maktab - malakali pedagogik jamoa',
  meta: [
    { name: 'description', content: '10-maktabning tajribali va malakali o\'qituvchilari. Matematika, fizika, kimyo, biologiya, tarix va boshqa fan o\'qituvchilari haqida ma\'lumot.' },
  ],
})
</script>
...
<template>
  <main>
    <PageHero title="Bizning o'qituvchilar" highlight="" subtitle="Maktabimizning tajribali va fidoyi ustozlari bilan tanishing." />
    ...
    <section class="py-20 bg-background min-h-screen">
      <div class="container mx-auto px-6 max-w-[1240px]">
        <div class="flex flex-col gap-12 mb-16 animate-on-scroll">
          <div class="bg-white p-8 rounded-3xl shadow-xl flex flex-col lg:flex-row gap-8 items-center justify-between">
            <div class="relative w-full lg:w-[450px]">
              <i aria-hidden="true" class="fas fa-search absolute left-6 top-1/2 -translate-y-1/2 text-slate-400"></i>
              <input type="text" v-model="searchQuery" placeholder="Ism yoki fan bo'yicha qidiruv..."
            ...
            <div class="flex flex-wrap justify-center gap-3 pb-2 lg:pb-0 w-full lg:w-auto">
              <button v-for="s in subjects" :key="s" @click="setFilter(s)"
                :class="['whitespace-nowrap px-6 py-3 rounded-xl font-bold transition-all shadow-sm', activeFilter === s ? 'bg-secondary text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200']">
                {{ subjectLabels[s] }}
              </button>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          <TeacherCard v-for="t in paginatedTeachers" :key="t.id" variant="active" :name="t.name" :subject="t.subject + ' o\'qituvchisi'" :image="t.image" :experience="t.experience" :telegram="t.telegram" :instagram="t.instagram" :linkTo="'/teachers/teacher-single/' + t.id" />
        </div>

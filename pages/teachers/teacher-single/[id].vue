...
useHead({
  title: computed(() => teacher.value ? `${teacher.value.name} | 10-Maktab` : 'O\'qituvchi profili | 10-Maktab - pedagog haqida'),
  meta: computed(() => teacher.value ? [
    { name: 'description', content: `${teacher.value.name} - ${teacher.value.subject} o'qituvchisi. ${teacher.value.experience}.` },
  ] : []),
})
</script>
...
<template>
  <main v-if="teacher">
    <PageHero gradient title="Ustoz" highlight="Profili" subtitle="Bizning ustozlarimiz bilan yaqinroq tanishing." />
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12">
            <div class="lg:col-span-4 bg-slate-50 p-12 border-r border-slate-100 flex flex-col items-center">
              <div class="relative w-full aspect-square rounded-[2.5rem] overflow-hidden shadow-2xl mb-10 border-8 border-white group">
                <img loading="lazy" :src="`/assets/images/${teacher.image}`" :alt="teacher.name"
                  class="w-full h-full object-cover group-hover:scale-110 transition-all duration-700">
                <div class="absolute inset-0 bg-gradient-to-t from-primary/60 via-transparent to-transparent opacity-40 group-hover:opacity-60 transition-all"></div>
              </div>
              ...
              <div class="w-full space-y-6">
                ...
                <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-5 group hover:shadow-md transition-all">
                  ...
                  <div>
                    <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Konsultatsiya</p>
            ...
            <div class="lg:col-span-8 p-12 lg:p-20">
              <div class="mb-12">
                <span class="inline-block px-5 py-2 bg-secondary text-white text-sm font-black rounded-full mb-6 shadow-lg shadow-secondary/20 uppercase tracking-widest">{{ teacher.subject }} o'qituvchisi</span>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="group">
                  <h4 class="text-primary font-black text-xl mb-6 flex items-center gap-3 italic">
                    <span class="w-8 h-1 bg-secondary rounded-full"></span> Mutaxassisligi:
                  </h4>
                ...
                <div class="group">
                  <h4 class="text-primary font-black text-xl mb-6 flex items-center gap-3 italic">
                    <span class="w-8 h-1 bg-secondary rounded-full"></span> Yutuqlari:
                  </h4>
                  <div class="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 group-hover:bg-white group-hover:shadow-xl group-hover:-translate-y-1 transition-all duration-500">
              ...
              <div class="mt-16 pt-16 border-t border-slate-100 flex flex-wrap items-center gap-10">
                <div class="flex items-center gap-4">
                  ...
                  <div class="text-slate-400 font-bold text-sm uppercase leading-tight">Yillik<br>tajriba</div>
                </div>
                <div class="flex items-center gap-4">
                  <div class="text-4xl font-black text-secondary italic">{{ teacher.students }}+</div>
                  <div class="text-slate-400 font-bold text-sm uppercase leading-tight">Muvaffaqiyatli<br>shogirdlar</div>
                </div>
              </div>
            </div>
          </div>

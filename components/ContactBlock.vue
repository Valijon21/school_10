...
v-if="variant === 'card'"
    ...
    <div
:class="`w-20 h-20 ${iconBg || 'bg-secondary/5'} ${iconColor || 'text-secondary'} rounded-[1.5rem] flex items-center justify-center text-3xl flex-shrink-0 group-hover:bg-${(iconBg?.match(/bg-(\w+)/) || ['', 'secondary'])[1]} group-hover:text-white transition-all duration-500`"
...
v-if="linkText" :href="linkHref || '#'"
        ...
        {{ linkText }} <i aria-hidden="true" class="fas fa-external-link-alt ml-2"/>
      </a>
    </div>
  </div>

  <li v-else-if="variant === 'footer'" class="flex items-center justify-center md:justify-start gap-4 text-slate-400">
    <i :class="['fas', icon, 'text-secondary w-5']" aria-hidden="true"/>
    <span>{{ lines[0] }}</span>
  </li>
